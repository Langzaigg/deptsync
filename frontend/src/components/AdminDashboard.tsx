...
const AdminDashboard: React.FC = () => {
    ...
    const handleEditUser = (user: User) => {
        setEditingUser(user);
        setEditFormData({ name: user.name, password: (user as any).password || '' });
        ...
        if (window.confirm(`确定将用户 ${freshUser.username} ${actionName} 吗?`)) {
            try {
                if (newRole === UserRole.ADMIN) {
                    await usersApi.promote(freshUser.id);
                } else {
                    const updatedUser: User = { ...freshUser, role: newRole };
                    await usersApi.update(freshUser.id, updatedUser);
                }
                await refreshData();
            } catch (error) {
    ...
    const handleGenerateMonthlyReport = async () => {
        ...
        try {
            const eventsPromises = projects.map(p =>
                eventsApi.getByProject(p.id, monthlyReportStartDate, monthlyReportEndDate)
            );
            const eventsArrays = await Promise.all(eventsPromises);
            const allEvents = eventsArrays.flat();

            const report = await llmApi.generateDeptMonthlyReport(monthlyReportStartDate, monthlyReportEndDate, projects, reports);

            setMonthlyReportContent(report);
        } catch (error) {
                                ...
                                {projects.map(p => {
                                    ...
                                    return (
                                        <tr key={p.id} className="hover:bg-slate-50/50">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${p.status === 'INITIATION' ? 'bg-blue-100 text-blue-700' :
