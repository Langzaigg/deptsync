...
import { initStorage, userService } from './services/storage';
...
interface AuthContextType {
  user: User | null;
  login: (jobNumber: string, password: string) => boolean;
  register: (name: string, jobNumber: string, password: string) => boolean;
  logout: () => void;
}
...
const App: React.FC = () => {
  ...
  const login = (jobNumber: string, password: string) => {
    const u = userService.login(jobNumber, password);
    if (u) {
      setUser(u);
      localStorage.setItem('deptsync_current_user', JSON.stringify(u));
      return true;
    }
    return false;
  };

  const register = (name: string, jobNumber: string, password: string) => {
    try {
      const u = userService.register(name, jobNumber, password);
      setUser(u);
      localStorage.setItem('deptsync_current_user', JSON.stringify(u));
      return true;
    } catch (e) {
