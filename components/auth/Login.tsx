...
import { UserCheck, Briefcase, Hash, Lock, User as UserIcon } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const success = isRegistering 
      ? register(name, jobNumber, password) 
      : login(jobNumber, password);
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full bg-brand-600 hover:bg-brand-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors shadow-md shadow-brand-100 mt-2"
          >
